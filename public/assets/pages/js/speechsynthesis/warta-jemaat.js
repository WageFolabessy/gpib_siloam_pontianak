document.addEventListener("DOMContentLoaded", function () {
    ...
    function getTextForElement(element) {
        ...
        if (
            ...
            element.classList.contains("warta-item") &&
    ...
    function stopAutoplay(cancelSpeech = true) {
        ...
        removeHighlight();
        if (cancelSpeech && synth.speaking) {
            synth.cancel();
        }
    }
    ...
    function initializePageTTS() {
        ...
        const wartaItems = document.querySelectorAll(".warta-item.speakable");
        const maxAutoplayWartaItems = 2;
        for (
            let i = 0;
            i < Math.min(wartaItems.length, maxAutoplayWartaItems);
            i++
        ) {
            autoplayQueue.push({
                element: wartaItems[i],
                text: getTextForElement(wartaItems[i]),
            });
        }
